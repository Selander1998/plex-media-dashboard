...
import {
	...
	REPORT_PATH, SERVER_SETTINGS_PATH, TORRENT_SAVE_PATHS,
} from "./config.js";
...
export async function processTorrent(torrent) {
	...
	try {
		if (type === "series") {
			for (const f of [...videoFiles, ...subtitleFiles]) {
				...
				// Only move files that carry a parseable episode number — skip bonus content, making-of, etc.
				if (!epInfo) {
					console.log(`[process] skip non-episode file: ${fname}`);
					continue;
				}
				// Skip featurette content even when it carries S##E## tags
				if (FEATURETTE_RE.test(fname)) {
					console.log(`[process] skip featurette: ${fname}`);
					continue;
				}
