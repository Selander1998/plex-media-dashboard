...
export async function processTorrent(torrent, { force = false } = {}) {
	...
	const cleanMovieName = movieInfo?.year ? `${movieInfo.title} (${movieInfo.year})` : movieInfo?.title ?? null;

	if (type === "movies" && !force && !movieInfo?.year) {
		console.log(`[process] Could not identify movie (no year): "${name}"`);
		await addQualityBlock(hash, name, ["unidentified:no_year"]);
		sendNtfy({ title: "Could not identify movie", body: `${name}\nNo year found — manual review required`, tags: "warning", priority: "high" });
		return;
	}

	const seriesInfo = type === "series" ? parseSeriesInfo(name) : null;
	if (type === "series" && !seriesInfo && !force) {
		const msg = "Could not parse show name from torrent name";
		console.error(`[process] ${msg}: "${name}"`);
		await addQualityBlock(hash, name, ["unidentified:no_episode_info"]);
		sendNtfy({ title: "Could not identify series", body: `${name}\n${msg}`, tags: "warning", priority: "high" });
		return;
	}
